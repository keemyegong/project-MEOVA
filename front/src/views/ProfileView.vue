...
const follow = function () {
  store.follow(route.params.username).then(() => {
    store.settings();
...
onMounted(() => {
  store.settings().then(() => {
    isFollow.value = store.profile_info.followers.includes(store.userinfo.pk);
  });
});
onBeforeRouteUpdate((to, from, next) => {
  store.profile(to.params.username); // 새로운 프로필 정보 가져오기
  store.settings(); // 새로운 설정 정보 가져오기
  next();
});
</script>
